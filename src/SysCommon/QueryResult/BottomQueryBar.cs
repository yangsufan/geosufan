...
namespace SysCommon
{
    public partial class BottomQueryBar : UserControl
    {
        private string LayerTreexmlpath = System.Windows.Forms.Application.StartupPath + "\\..\\res\\xml\\查询图层树_QueryBar.xml"; //图层目录文件路径
        private string m_DicPath = System.Windows.Forms.Application.StartupPath + "\\..\\Template\\Dic.gdb";
        public BottomQueryBar()
        {
            InitializeComponent();
            //comboLayers.ReadOnly = true;
        }
        
        private List<string> _ListDataNodeKeys = null;  //added by chulili 2012-11-13 用户的数据权限列表，选择图层给出图层列表的时候用，列表中仅显示有权限的图层
        ...
        private string _DefaultNodeKey = "";    //added by chulili 2012-11-08 默认第一个展示的图层
        ...
        //added by chulili 2012-11-14 查询类型枚举
        internal enum enumQueryType
        {
            enumGeometryQuery,  //几何类型查询，包含多边形查询、多边形缓冲查询
            enumSQLQuery,       //自定义查询，即指定sql条件查询
            enumDomainQuery     //地名查询，即指定几个图层，指定sql条件查询，这里的sql条件略复杂，支持查询结果按符合程度排序等
        }

        private enumQueryType _CurQueryType;    //added by chulili 2012-11-14 当前的查询类型

        private IQueryFilter _CurQueryFilter = null;//added by chulili 2012-11-14  用于表示当前查询的过滤条件
        //added by chulili 2012-11-14 地名查询参数
        ...
        private string _CurNodeKey = "";  //added by chulili 2012-11-14 当前查询层的NodeKey
        //初始化表结构 
        //ygc 2012-08-09
        ...
        public void EmergeQueryData(IMap pMap, IGeometry pGeometry, esriSpatialRelEnum pesriSpatialRelEnum)
        {
           ...
           // m_dataSourceGrid.Rows.Clear();
            ...
            while (pLayer != null)
            {
                //changed by chulili 为什么只能查到可见的图层？改成全部可查
                //if ((pLayer.MaximumScale <= pMap.MapScale || pLayer.MaximumScale == 0) && (pLayer.MinimumScale == 0 || pMap.MapScale <= pLayer.MinimumScale) && pLayer.Visible == true)//20110802
                //{
