...
namespace SysCommon
{
    public static class ModuleMap
    {
        ...
        //added by chulili 20110730 根据nodeKey查找图层c
        //bOnlyFromMap参数含义：是否仅仅从Map中查找  true仅从map中查找 false从数据源中查找，根据xml构造layer        
        public static ILayer GetLayerByNodeKey(IWorkspace pTmpWorkSpace, IMap pMap, string strNodeKey, XmlDocument LayerTreeXmlDoc, bool bOnlyFromMap)
        {
            ...
            if (!bOnlyFromMap)  //如果不仅从视图中获取，还要从配置文件中获取并构造layer
            {
                Exception err = null;
        ...
        //added by chulili 20111119 获取当前比例尺是否图层的可见比例尺
