...
{
    /// <summary>
    /// cyf 20110615  add:初始化rastercatalog,选择服务器上的数据，或者外部的数据初始化Rastercatalog
    /// 若不是服务器上的数据，首先将数据上传到FTP服务器上；第二步，将ftp服务器上的数据录入到RasterCatalog里面
    /// </summary>
    public class ControlsInitRasterCatalog : Plugin.Interface.CommandRefBase
    {
        ...
        //初始化按钮信息
        public ControlsInitRasterCatalog()
        {
            base._Name = "GeoDBATool.ControlsInitRasterCatalog";
            base._Caption = "栅格数据入库(RC)";
            base._Tooltip = "栅格数据入库(RC)";
            base._Visible = true;
            base._Enabled = true;
            base._Message = "栅格数据入库(RC)";
        ...
        public override void OnClick()
        {
            ...
            //进行初始化设置
            FrmInitiaRasterCatalog pFrmInitiaRasterCatalog = new FrmInitiaRasterCatalog(ModData.TempWks, m_Hook, EnumRasterOperateType.Input);
