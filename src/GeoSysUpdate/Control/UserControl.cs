...
namespace GeoSysUpdate
{
    public partial class UserControlSMPD : UserControl
    {
        ...
        private SysCommon.BottomQueryBar _BottomQueryBar;
        ...
        private string _Decimalstr = ""; //added by chulili 状态栏当前比例尺小数位数格式化参数
        /// ZQ 20111103
        ...
        public UserControlSMPD(string strName, string strCation)
        {
            ...
            pAppFrm.TOCControl = axTOCControl.Object as ITOCControlDefault;
            pAppFrm.MapControl = axMapControl.Object  as IMapControlDefault;
            ...
            //ModData.v_AppGisUpdate.ResultFileTree = advTreeResultFileList;//deleted by chulili 20111104 三个项目都用不到成果列表界面了 added by chulili 20110923 修改了底层 成果列表树图
            ModData.v_AppGisUpdate.LayerAdvTree = this.DataViewTree.DataTree as object; //added by chulili 20111129 将图层树的AdvTree开放出去

            ModData.v_AppGisUpdate.LayerTree = this.DataViewTree as object;
            SysCommon.ModSysSetting.CopyLayerTreeXmlFromDataBase(Plugin.ModuleCommon.TmpWorkSpace, _layerTreePath);
            ModData.v_AppGisUpdate.LayerTreeXmlPath = _layerTreePath;
            ...
            ModData.v_AppGisUpdate.QueryBar = _BottomQueryBar;
            if (ModData.v_AppGisUpdate.QueryBar.m_WorkSpace  == null)
            {
                ModData.v_AppGisUpdate.QueryBar.m_WorkSpace = Plugin.ModuleCommon.TmpWorkSpace;
            }
            ...
            //xisheng 20120612 添加制图文件目录树初始化 start
            ModData.v_AppGisUpdate.ResultsTree = ucResultsTree.ResultsTree; //zq add 成果目录树
            ucResultsTree.MapControlDefault = axMapControl.Object as IMapControlDefault;
            ucResultsTree.pWorkspace = Plugin.ModuleCommon.TmpWorkSpace;//业务库传入
            ucResultsTree.InitializeResultsTree();//初始化成果目录树
            ucResultsTree.NodeMouseDownCilck += new GeoSysUpdate.Control.myEventHandler(ucResultsTree_NodeMouseDownCilck);
            //xisheng 20120612 制图文件目录树 end
        ...
        void ucResultsTree_NodeMouseDownCilck(object sender, TreeNodeMouseClickEventArgs e)
        {
            ucResultsTree.ResultsTree.SelectedNode = e.Node;

            //右键点击的时候弹出右键菜单
            if (e.Button == MouseButtons.Right)
            {
                ucResultsTree.ResultsTree.SelectedNode = e.Node;
                System.Drawing.Point ClickPoint = ucResultsTree.ResultsTree.PointToScreen(new System.Drawing.Point(e.X, e.Y));
                TreeNode tSelNode;
                tSelNode = e.Node;
                if (tSelNode != null)
                {
                    ...
                    DevComponents.DotNetBar.ContextMenuBar menuBar = _dicContextMenu["ContextMenuresultsTree"];
                    this.Controls.Add(menuBar);
                    if (_dicContextMenu.ContainsKey("ContextMenuresultsTree"))
                    {
                        if (_dicContextMenu["ContextMenuresultsTree"] != null)
                        {
                            if (_dicContextMenu["ContextMenuresultsTree"].Items.Count > 0)
                            {
                                item = _dicContextMenu["ContextMenuresultsTree"].Items[0] as DevComponents.DotNetBar.ButtonItem;
                                if (item != null)
                                {
                                    item.Popup(ucResultsTree.ResultsTree.PointToScreen(pPoint));
                                }
                            }
                        }
                    }
        ...
        private void InitDataView()
        {
            ...
            //从数据库中拷贝图层树
            SysCommon.ModSysSetting.CopyLayerTreeXmlFromDataBase (ModData.v_AppGisUpdate.CurWksInfo.Wks, _layerTreePath);
            ...
            if (_dicContextMenu.ContainsKey("LayerTreeNodeContextMenu"))
            {
                DevComponents.DotNetBar.ContextMenuBar menuBarLayer = _dicContextMenu["LayerTreeNodeContextMenu"];
            ...
            if (!File.Exists(ModData._XZQpath))
            {
                return;
            }
            try
            {
                InitFromXZQXML(ModData._XZQpath);
                InitFromLCXML(ModData._XZQpath);
            }
            catch (Exception ex)
            {
        ...
        {
            //axTOCControl.SetBuddyControl(null);//yjl20111102 add 加载数据前 解绑TOC
            //褚丽丽添加 初始化数据源字典（由于这个步骤有时耗时较长，所以单独提出来执行，只需初始化一次即可）
            this.DataViewTree.InitDBsource();
            this.DataViewTree.LoadData();
            //axTOCControl.SetBuddyControl(axMapControl);//yjl20111102 add 加载数据后 绑定TOC（200图层效率快1倍）
        }
        ...
        private void InitIslandTree()
        {
            ...
            pXmlDoc.Load(_RoadandRiverXmlPath);
            //读取
            string strSearch = "//IslandRoot";
            XmlNode pXmlNode = pXmlDoc.SelectSingleNode(strSearch);
            if (pXmlNode == null)
                return;
            //道路根节点
            DevComponents.AdvTree.Node pNode = new DevComponents.AdvTree.Node();
            pNode.Name = "IslandRoot";
            pNode.ImageIndex = 35;
            //pNode.SelectedImageIndex = 35;
            pNode.Text = pXmlNode.Attributes["ItemName"].Value;
            pNode.Tag = pXmlNode.OuterXml as object;
            ...
            for (int i = 0; i < pXmlNode.ChildNodes.Count; i++)
            {
                ...
                {
                    DevComponents.AdvTree.Node pTmpNode = new DevComponents.AdvTree.Node();
                    pTmpNode.Name = pTmpXmlNode.Attributes["IslandCode"].Value;
                    pTmpNode.ImageIndex = 34;
                    pTmpNode.Tag = "Island";
                    pTmpNode.DataKey = pTmpXmlNode.Attributes["Path"].Value; ;
                    //pTmpNode.SelectedImageIndex = 34;
                    pTmpNode.Text = pTmpXmlNode.Attributes["ItemName"].Value;
                    pNode.Nodes.Add(pTmpNode);
                }
        ...
        private void InitialMainViewControl()
        {
            ...
            newfrmBarManager.PanelLyr.Controls.Add(this.tabControlResult);//直接加到界面上去
            this.tabControlResult.Dock = DockStyle.Fill;
            ...
            /*   PanelDockContainer PanelTipErr = new PanelDockContainer();
               ...
               DockContainerItem dockItemDataCheck = new DockContainerItem("dockItemDataCheck", "查询结果");
        ...
        private void InitialFrmDefControl()
      {
        ...
        private void axTOCControl_OnMouseDown(object sender, ITOCControlEvents_OnMouseDownEvent e)
        {
            ...
            {
                DevComponents.DotNetBar.ButtonItem item = null;
                DevComponents.DotNetBar.ContextMenuBar menuBar = _dicContextMenu["TOCContextMenu1"];
                DevComponents.DotNetBar.ContextMenuBar menuBarLayer = _dicContextMenu["TOCLayerContextMenu1"];
                this.Controls.Add(menuBar);
                this.Controls.Add(menuBarLayer);
                switch (TOCItem)
                {
                    case esriTOCControlItem.esriTOCControlItemMap:
                        if (_dicContextMenu.ContainsKey("TOCContextMenu1"))
                        {
                            if (_dicContextMenu["TOCContextMenu1"] != null)
                            {
                                if (_dicContextMenu["TOCContextMenu1"].Items.Count > 0)
                                {
                                    item = _dicContextMenu["TOCContextMenu1"].Items[0] as DevComponents.DotNetBar.ButtonItem;
                                    if (item != null)
                                    {
                                        item.Popup(axTOCControl.PointToScreen(pPoint));
                                    }
                    ...
                    case esriTOCControlItem.esriTOCControlItemLayer:
                        ...
                        {
                            IFeatureLayer pFeatureLayer = pLayer as IFeatureLayer;
                            if (_dicContextMenu.ContainsKey("TOCLayerContextMenu1"))
                            {
                                if (_dicContextMenu["TOCLayerContextMenu1"] != null)
                                {
                                    if (_dicContextMenu["TOCLayerContextMenu1"].Items.Count > 0)
                                    {
                                        item = _dicContextMenu["TOCLayerContextMenu1"].Items[0] as DevComponents.DotNetBar.ButtonItem;
                                        if (item != null)
                                        {
                                            item.Popup(axTOCControl.PointToScreen(pPoint));
                                        }
        ...
        private void axMapControl_OnMouseDown(object sender, IMapControlEvents2_OnMouseDownEvent e)
        {
            ...
            if (e.button == 1 || _dicContextMenu == null || e.button == 4)
            {
                return;
            }
            if (axMapControl.CurrentTool != null)
            {
                if (axMapControl.CurrentTool.ToString().Equals("GeoUtilities.ControlsMapMeasureToolDefClass"))
                {
                    return;
                }
            }
            ...
            System.Drawing.Point pPoint = new System.Drawing.Point(e.x, e.y);
            DevComponents.DotNetBar.ButtonItem item = null;
            //yjl20111114 add 捕捉右键菜单
            if (axMapControl.CurrentTool != null)
            {
                if (axMapControl.CurrentTool.ToString().Equals("GeoDataEdit.CreatePolygonTool") || axMapControl.CurrentTool.ToString().Equals("GeoDataEdit.CreatePolylineTool"))
                {
                    if (_dicContextMenu.ContainsKey("MapControlContextMenuSnap"))
                    {
                        if (_dicContextMenu["MapControlContextMenuSnap"].Items.Count > 0)
                        {
                            DevComponents.DotNetBar.ContextMenuBar menuBar = _dicContextMenu["MapControlContextMenuSnap"];
                            this.Controls.Add(menuBar);
        ...
        {
            tabControlResult.SelectedTab = tabItemJHTable;
        }
        //added by chulili 20110802 切换到行政区tab页
        public void TurnToXZQTab()
        {
            tabControlResult.SelectedTab = tabItemXZQ;
        }
        public void AdjustLayerOrder()
        {
            tabControlResult.SelectedTab = tabItemDBLayer;
        }
        /// <summary>
        /// 控件切换
        /// </summary>
        ...
        private void tabControlInfo_SelectedTabChanged(object sender, TabStripTabChangedEventArgs e)
        {
            ...
            if (tabControlInfo.SelectedTab.Name.Equals("tabItemMapView"))
            {
                ...
                ModData.v_AppGisUpdate.CurrentControl = axMapControl.Object;
                //  张琪添加    20110709
                tabControlResult.SelectedTab = tabItemDBSource; //changed by chulili 20110830 tabItemDBLayer->tabItemDBSource 图层目录已隐藏
            }
            else if (tabControlInfo.SelectedTab.Name.Equals("tabSceneView"))
            {
                ...
                //tabControlResult.SelectedTab = tabItem3DLayer;
            ...
            else if (tabControlInfo.SelectedTab.Name.Equals("tabMetadataView"))//tabMetadataView 元数据视图 //shduan 20110619*****************
            {
                ...
                //  张琪添加
 				if (tabControlResult.SelectedTab != tabItemJHTable)//added by chulili 20110727  添加判断，如果本来就是元数据页面，就不用再初始化
                {
                    tabControlResult.SelectedTab = tabItemJHTable;
                     ////ZQ  20111017  取消元数据结果重置
        ...
        //从xml文件初始化林场树
        private void InitFromLCXML(string cPath)
        {
            XmlDocument cXmlDoc = new XmlDocument();
            ...
            if (cXmlDoc != null)
            {
                ...
                vRootNode.ImageIndex = 38;
                vRootNode.DataKey = vChinaNode as object;
                treeViewLC.Nodes.Add(vRootNode);//行政区节点
                DevComponents.AdvTree.Node tNode1 = treeViewLC.Nodes[0];
                if (!vChinaNode.HasChildNodes)
                    return;
                vRootNode.Expand();
                foreach (XmlNode vSubXmlNode in vChinaNode.ChildNodes)//全国0//改为省节点
        ...
        public IGeometry getExtentByXZQ(DevComponents.AdvTree.Node vSelectNode)
        {
            string strTag = vSelectNode.Tag.ToString();
            if (strTag.ToUpper() == "TOWN")
            {
                //return getExtentByXIANG(vSelectNode);
            }
            if (File.Exists(ModData._XZQpath))
            {
                XmlDocument pXmldoc = new XmlDocument();
                pXmldoc.Load(ModData._XZQpath);
        ...
        private IGeometry getExtentByXIANG(DevComponents.AdvTree.Node vSelectNode)
        {
            string strTag = vSelectNode.Tag.ToString();
            if (strTag.ToUpper() != "TOWN")
            {
            ...
            DevComponents.AdvTree.Node vPnode = vSelectNode.Parent;
            string strPtag = vPnode.Tag.ToString();
            if (File.Exists(ModData._XZQpath))
            {
                XmlDocument pXmldoc = new XmlDocument();
                pXmldoc.Load(ModData._XZQpath);
                ...
                if (pEle != null)
                {
                    ...
                    IFeatureClass pFeatureClass = ModDBOperate.GetFeatureClassByNodeKey(strNodeKey);
                    XmlNode pPnode = pXmldoc.SelectSingleNode("//LayerConfig/" + strPtag);
                    string strPXZBM = pPnode.Attributes["XZBMField"].Value.ToString();
                    pXmldoc = null;
                    string strWhere = strXZBM+"='"+vSelectNode.Name+"' and "+strPXZBM+" ='"+vPnode.Name+"'";
                    return getXZQExtentFromWhere(pFeatureClass, strWhere);
                }
                pXmldoc = null;
        ...
        //根据行政区图层，行政编码字段名称和字段值，获取对应的地物几何数据
        private IGeometry getXZQExtentFromWhere(IFeatureClass pXZQFeaCls, string WhereClause)
        {
        ...
        private void DealRoadorRiverLocation(DevComponents.AdvTree.Node pNode)
        {
            ...
            if (pParentNode.Name != "RoadRoot" && pParentNode.Name != "RailRoadRoot" && pParentNode.Name != "RiverRoot" && pParentNode.Name != "IslandRoot")
                return;
            //配置文件是否存在
            if (!File.Exists(_RoadandRiverXmlPath))
            {
            ...
            //构造查询条件
            ///******xisheng20120618  原有查询方式更改为属性查询，不明白之前为什么用空间查询 start******
            ...
            if (strFieldValue.Contains(","))
            {
                pQueryFilter.WhereClause = strField + " in ('" + strFieldValue + "')";
            }
            else
            {
                pQueryFilter.WhereClause = strField + "='" + strFieldValue + "'";
            }
            IFeatureCursor pFeaCursor = pFeatureClass.Search(pQueryFilter, false);
            ///******xisheng20120618  原有查询方式更改为属性查询，不明白之前为什么用空间查询 end******
            ...
            SysCommon.ModPublicFun.ResizeEnvelope(pEnvelope,1.2);
            axMapControl.Extent = pEnvelope;
            //drawPolyLineElement(pListGeometry, psGra);
            axMapControl.Refresh();
            ///修改要素绘制效果   ZQ 20111102 modify 
            ModData.v_AppGisUpdate.pGeometry = ModDBOperator.GetLyrUnionPlygon(pListGeometry);
            axMapControl.ActiveView.ScreenDisplay.UpdateWindow();
            axMapControl.FlashShape(ModData.v_AppGisUpdate.pGeometry, 3, 200, null);
            drawgeometryXOR(ModData.v_AppGisUpdate.pGeometry);
            axMapControl.ActiveView.PartialRefresh(esriViewDrawPhase.esriViewForeground, null, axMapControl.ActiveView.Extent);
            pListGeometry.Clear();
        ...
        public void LocationByXZQNode()
        {
            try
            {
                ///ZQ 20111020  add 增加最高行政区定位到全图
                DevComponents.AdvTree.Node pNode = treeViewXZQ.SelectedNode;
                if (pNode.Parent != null)
                {
                    DevComponents.AdvTree.Node pParentNode = pNode.Parent;
                    switch (pParentNode.Name)
                    {
                        case "行政区":
                            ESRI.ArcGIS.SystemUI.ICommand pComm = new ESRI.ArcGIS.Controls.ControlsMapFullExtentCommand();
                            pComm.OnCreate(axMapControl.Object);
                            pComm.OnClick();
                            return;
                    }
                }
                ...
                ModData.v_AppGisUpdate.pGeometry = null;
                IEnvelope pExtent = pGeometry.Envelope;
                ...
                ModData.v_AppGisUpdate.pGeometry = pTopologicalOperator.Boundary;
                axMapControl.FlashShape(pGeometry, 3, 200, null);
                drawgeometryXOR(ModData.v_AppGisUpdate.pGeometry);
            }
            catch (Exception ex)
       ...
       //根据图幅号定位到图形信息    ZQ  20110728  add      20110730  modify
        ...
        private void UserControlSMPD_EnabledChanged(object sender, EventArgs e)
        {
            ...
            {
                //初始化图层视图
                SysCommon.CProgress vProgress = new SysCommon.CProgress("数据展示管理");
                ...
                //@@@ModDBOperator.WriteLog("UserControlSMPD_EnabledChanged true 鹰眼");
                try
                {
                    for (int i = 0; i < pFC.Count; i++)
                    {
        ...
        {
            /////被擦出重绘制  ZQ 2011101 
            if (e.viewDrawPhase ==32 && ModData.v_AppGisUpdate.pGeometry != null)
            {
                drawgeometryXOR(ModData.v_AppGisUpdate.pGeometry);
            }
            ...
            this.DataViewTree.UpdateLayerNodeImage();
            //this.Cursor = Cursors.Default;
            ///ZQ 2011 1126  add
            foreach (var item in SysCommon.ScreenDraw.list)
            {
                item(e.display as IDisplay, (esriViewDrawPhase)e.viewDrawPhase);
            }
        }
        ...
        private void treeViewXZQ_NodeClick(object sender, DevComponents.AdvTree.TreeNodeMouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Right)
            {
                ...
                //    cntxtJMP.Show(e.Location);
                //}//ZQ    20110731   modify
                if (tSelNode != null && tSelNode.Level >= 1 && tSelNode.Tag != null)
                {
                    if (tSelNode.Tag.ToString() == "Province" || tSelNode.Tag.ToString() == "City" || tSelNode.Tag.ToString() == "County" || tSelNode.Tag.ToString() == "Town" || tSelNode.Tag.ToString() == "Village")
                    {
                        //cntxtXZQ.TopLevel = false;
                        //cntxtXZQ.Parent = treeViewXZQ as Control;
                        //cntxtXZQ.Show(e.Location);
                    ...
                    else if (tSelNode.Tag.ToString() == "Island")//海岛礁右键菜单
                    {
                        System.Drawing.Point pPoint = new System.Drawing.Point(e.X, e.Y);
                        DevComponents.DotNetBar.ButtonItem item = null;
                        DevComponents.DotNetBar.ContextMenuBar menuBar = _dicContextMenu["ContextMenuIsland"];
                        this.Controls.Add(menuBar);
                        if (_dicContextMenu.ContainsKey("ContextMenuIsland"))
                        {
                            if (_dicContextMenu["ContextMenuIsland"] != null)
                            {
                                if (_dicContextMenu["ContextMenuIsland"].Items.Count > 0)
                                {
                                    item = _dicContextMenu["ContextMenuIsland"].Items[0] as DevComponents.DotNetBar.ButtonItem;
                                    if (item != null)
                                    {
                                        item.Popup(treeViewXZQ.PointToScreen(pPoint));
                                    }
        ...
        private void treeViewXZQ_NodeDoubleClick(object sender, DevComponents.AdvTree.TreeNodeMouseEventArgs e)
        {
            ...
            try
            {
                ...
                ///ZQ 20111020 定位范围扩大1.5倍
                SysCommon.ModPublicFun.ResizeEnvelope(pExtent, 1.5);
                //axMapControl.ActiveView.Deactivate();
                (axMapControl.Map as IActiveView).Extent = pExtent;
                 ////ZQ    20110914    modify   改变显示方式
                ////drawPolygonElement(pGeometry as IPolygon, psGra);
                //axMapControl.ActiveView.PartialRefresh(esriViewDrawPhase.esriViewForeground, null, null);
        ...
        private void drawgeometryXOR(IGeometry pGeometry)
       {
            ...
            IScreenDisplay pScreenDisplay = axMapControl.ActiveView.ScreenDisplay;
            ISymbol pSymbol = null;
            //颜色对象
            IRgbColor pRGBColor = new RgbColorClass();
            pRGBColor.UseWindowsDithering = false;
            pRGBColor = ModDBOperator.getRGB(255,0, 0);
        ...
        private void axMapControl_OnViewRefreshed(object sender, IMapControlEvents2_OnViewRefreshedEvent e)
       {
        ...
        {
            this.Cursor = Cursors.WaitCursor;//地图刷新的时候处于等待状态 xisheng 20111101 
            //axMapControl.MousePointer = esriControlsMousePointer.esriPointerHourglass;
        }

        private void axMapControl_OnAfterScreenDraw(object sender, IMapControlEvents2_OnAfterScreenDrawEvent e)
        {
            this.Cursor = Cursors.Default;//地图刷新完的时候结束等待 xisheng 20111101 
