...
{
    public class CommandProtectGradeStatistics : Plugin.Interface.CommandRefBase
    {
        private Plugin.Application.IAppGisUpdateRef m_Hook;
        public CommandProtectGradeStatistics()
        {
            base._Name = "GeoDataManagerFrame.CommandProtectGradeStatistics";
            base._Caption = "保护等级统计";
            base._Tooltip = "保护等级统计";
            base._Checked = false;
            base._Visible = true;
            base._Enabled = true;
            base._Message = "保护等级统计";
        }
        ...
        public override void OnClick()
        {
            ...
            try
            {//获取地名地物类
                ModQuery.GetPlaceNameStatisticsConfig(out pZLDJFeaClass, out ZLDJLayerName, out strZLDJ, "林地保护等级分布");
                ...
                if (pZLDJFeaClass != null)
                {
                    ...
                    {
                        MessageBox.Show("找不到林地图斑数据保护等级属性,请检查配置文件!", "提示", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        pZLDJFeaClass = null;
                        return;
                    }
                }
                ...
                if (pfrmSetStatistic.ShowDialog() == DialogResult.OK)
                {
                    ...
                    pExportToExcel.Export(pZLDJFeaClass, pFeatureCursor, "林地保护等级分布", "林地保护等级", strZLDJ);
                    System.Runtime.InteropServices.Marshal.ReleaseComObject(pFeatureCursor);
                }
                if (this.WriteLog)
                {
                    Plugin.LogTable.Writelog("专题图统计-林地保护等级统计");//ygc 2012-9-14 写日志
                }
