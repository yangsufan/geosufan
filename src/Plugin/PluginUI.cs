...
namespace Fan.Plugin
{
    ...
    public  class PluginUI
    {
        ...
        public PluginStruct m_AllPlugin
        {
        ...
        {
            if (ribbonPageCategory == null) return string.Format("无父控件");
            ...
            if (pSelectRows.Length <= 0) return string.Format("无权限");
        ...
        /// <returns></returns>
        private string InitialRibbonPageGroup(RibbonPage ribbonPage)
        {
            if (ribbonPage == null) return string.Format("无父控件");
            string strFid = ribbonPage.Tag.ToString();
            DataRow[] pSelectRows = _AppUser.UserRole.RoleFunDt.Select(string.Format("{0}='{1}' and {2}='{3}'",
                ColumnName.PID, strFid,ColumnName.ControlType,PluginControlType.RibbonPageGroup.ToString()));
            if (pSelectRows.Length <= 0) return string.Format("无权限");
            foreach (DataRow pRow in pSelectRows)
            {
                ...
                ribbonPageGroup.Name = FName;
                ribbonPageGroup.Text = pRow[ColumnName.Caption].ToString();
                ribbonPageGroup.Tag = pRow[ColumnName.FID];
                //初始化PageGroup下的按钮以及其他控件
                InitialBarItem(ribbonPageGroup);
            }
            return string.Empty;
        }
        private string InitialBarItem(RibbonPageGroup ribbonPageGroup)
        {
            if (ribbonPageGroup == null) return string.Format("无父控件");
            string strFid = ribbonPageGroup.Tag.ToString();
            DataRow[] pSelectRow = _AppUser.UserRole.RoleFunDt.Select(string.Format("{0}='{1}'",
                ColumnName.PID, strFid));
            if (pSelectRow.Length <= 0) return string.Format("无权限");
            foreach (DataRow pRow in pSelectRow)
            {
                string strControlType = pRow[ColumnName.ControlType].ToString();
                PluginControlType ControlTpe = (PluginControlType)Enum.Parse(typeof(PluginControlType), strControlType);
                string FName= pRow[ColumnName.FName].ToString();
                if (AllPlugin.dicPlugins.ContainsKey(FName))
                {
                    PluginOnCreate(AllPlugin.dicPlugins[FName], _pIAppFrm);
                ...
                switch (ControlTpe)
                {
                    case PluginControlType.BarButtonItem:
                        BarButtonItem item = new BarButtonItem();
                        item.Name = FName;
                        item.Caption= pRow[ColumnName.Caption].ToString();
                        item.Tag = pRow[ColumnName.FID];
        ...
        {
            BarButtonItem aBaseItem = e.Item as BarButtonItem;
            string sKey = aBaseItem.Name.ToString().Trim();
            if (m_pBaseItem == null)
            {
                m_pBaseItem = aBaseItem as BarButtonItem;
            }
            else
            {
