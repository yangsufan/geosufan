...
namespace GeoDataChecker
{
    ...
    /// 控制点注记一致性检查
    /// </summary>
    public class ControlPointAnnCheck:Plugin.Interface.CommandRefBase
    {
        private Plugin.Application.IAppGISRef _AppHk;

        public ControlPointAnnCheck()
        {
            base._Name = "GeoDataChecker.ControlPointAnnCheck";
            base._Caption = "控制点注记检查";
            base._Tooltip = "检查图面中的控制点的z值是否和控制点的高程注记的内容相一致";
            base._Checked = false;
            base._Visible = true;
            base._Enabled = true;
            base._Message = "控制点注记检查";
        }
        ...
        {
            //执行控制点注记检查
            //FrmMathematicsCheck mFrmMathematicsCheck = new FrmMathematicsCheck(_AppHk, enumErrorType.控制点注记一致性);
        ...
        private void PointAnnoCheck(DataCheckClass pDataCheckClass, List<IFeatureClass> LstFeaClass, DataTable pTable, string codeName, double radiu, long precision, out Exception eError)
        {
            ...
            for (int i = 0; i < pTable.Rows.Count; i++)
            {
                ...
                //改变树图运行状态
                pDataCheckClass.ChangeTreeSelectNode(pNode, "完成图层" + oriFeaClsName + "控制点注记一致性检查！", false);
            }
        }
    }
}
