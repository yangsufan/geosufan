...
namespace GeoDataChecker
{
    ...
    /// 面边界面边界重合检查
    /// </summary>
    public class GeoCheckAreaBoundaryCoveredByAreaBoundary : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //面边界面边界重合检查，参数为41
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 41, out eError);
                if (eError != null)
                {
                ...
                AreaTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTAreaBoundaryCoveredByAreaBoundary, out eError);
                if (eError != null)
                {
                    eError = new Exception("面边界面边界重合检查失败！" + eError.Message);
                    return;
                }
        ...
        public void DataCheckRealize_DataErrTreat(object sender, DataErrTreatEvent e)
        {
            ...
            e.ErrInfo.FunctionName = "面边界面边界重合检查";
            e.ErrInfo.ErrID = enumErrorType.面边界面边界重合检查.GetHashCode();
