...
namespace GeoDataChecker
{
    ...
    /// 面面相互覆盖检查
    /// </summary>
    public class GeoCheckAreaCoverEachOther : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //面面相互覆盖检查，参数为42
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 42, out eError);
                if (eError != null)
                {
                ...
                AreaTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTAreaAreaCoverEachOther,out eError);
                if (eError != null)
                {
                    eError = new Exception("面面相互覆盖检查失败！" + eError.Message);
                    return;
                }
        ...
        public void DataCheckRealize_DataErrTreat(object sender, DataErrTreatEvent e)
        {
            ...
            e.ErrInfo.FunctionName = "面面相互覆盖检查";
            e.ErrInfo.ErrID = enumErrorType.面面相互覆盖检查.GetHashCode();
