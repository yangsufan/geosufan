...
namespace GeoDataChecker
{
    ...
    /// 线面重合检查
    /// </summary>
    public class GeoLineCoveredByAreaBoundaryCheck : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //线面重合检查，参数ID为16
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 16, out eError);
                if (eError != null)
                {
                ...
                AreaTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTLineCoveredByAreaBoundary, out eError);
                if (eError != null)
                {
                    eError=new Exception("线面重合检查失败！" + eError.Message);
                    return;
                }
        ...
        public void DataCheckRealize_DataErrTreat(object sender, DataErrTreatEvent e)
        {
            ...
            e.ErrInfo.FunctionName = "线面重合检查";
            e.ErrInfo.ErrID = enumErrorType.线面边界重合检查.GetHashCode();
