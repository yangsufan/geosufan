...
namespace GeoDataChecker
{
    ...
    /// 异层面重叠检查
    /// </summary>
    public class GeoPolygonOverlapPolygonCheck : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //异层面重叠检查
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 7, out eError);
                if (eError != null)
                {
                ...
                AreaTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTAreaNoOverlapArea, out eError);
                if (eError != null)
                {
                    eError = new Exception("异层面重叠检查失败！" + eError.Message);
                    return;
                }
        ...
        public void DataCheckRealize_DataErrTreat(object sender, DataErrTreatEvent e)
        {
            ...
            e.ErrInfo.FunctionName = "异层面重叠检查";
            e.ErrInfo.ErrID = enumErrorType.异层面重叠检查.GetHashCode();
