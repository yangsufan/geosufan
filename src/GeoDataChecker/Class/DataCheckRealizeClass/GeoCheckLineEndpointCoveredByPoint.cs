...
namespace GeoDataChecker
{
    ...
    /// 线端点被点覆盖检查
    /// </summary>
    public class GeoCheckLineEndpointCoveredByPoint : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //线端点被点覆盖检查，参数为46
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 46, out eError);
                if (eError != null)
                {
                ...
                AreaTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTLineEndpointCoveredByPoint, out eError);
                if (eError != null)
                {
                    eError = new Exception("线端点被点覆盖检查失败！" + eError.Message);
                    return;
                }
        ...
        public void DataCheckRealize_DataErrTreat(object sender, DataErrTreatEvent e)
        {
            ...
            e.ErrInfo.FunctionName = "线端点被点覆盖检查";
            e.ErrInfo.ErrID = enumErrorType.线端点被点覆盖检查.GetHashCode();
