...
namespace GeoDataChecker
{
    ...
    /// 面含面检查
    /// </summary>
    public class GeoAreaCoveredByAreaCheck : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //面含面检查，参数ID为9
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 9, out eError);
                if (eError != null)
                {
                ...
                AreaTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTAreaCoveredByArea, out eError);
                if (eError != null)
                {
                   eError=new Exception("面含面检查失败！" + eError.Message);
                    return;
                }
