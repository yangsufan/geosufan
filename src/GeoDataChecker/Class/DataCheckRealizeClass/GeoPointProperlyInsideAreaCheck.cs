...
namespace GeoDataChecker
{
    ...
    /// 点位于面内检查
    /// </summary>
    public class GeoPointProperlyInsideAreaCheck : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //面含点检查，参数ID为13
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 13, out eError);
                if (eError != null)
                {
                ...
                AreaTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTPointProperlyInsideArea, out eError);
                if (eError != null)
                {
                    eError = new Exception("点位于面内检查失败！" + eError.Message);
                    return;
                }
        ...
        public void DataCheckRealize_DataErrTreat(object sender, DataErrTreatEvent e)
        {
            ...
            e.ErrInfo.FunctionName = "点位于面内检查";
            e.ErrInfo.ErrID = enumErrorType.点位于面内检查.GetHashCode();
