...
namespace GeoDataChecker
{
    ...
    /// 点搭面检查
    /// </summary>
    public class GeoPointCoveredByBoundaryCheck : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //点搭面检查，参数ID为10
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 10, out eError);
                if (eError != null)
                {
                ...
                AreaTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTPointCoveredByAreaBoundary, out eError);
                if (eError != null)
                {
                    eError=new Exception("点搭面检查失败！" + eError.Message);
                    return;
                }
        ...
        public void DataCheckRealize_DataErrTreat(object sender, DataErrTreatEvent e)
        {
            ...
            e.ErrInfo.FunctionName = "点搭面检查";
            e.ErrInfo.ErrID = enumErrorType.点搭面检查.GetHashCode();
