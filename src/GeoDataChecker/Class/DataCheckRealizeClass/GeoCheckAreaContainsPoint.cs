...
namespace GeoDataChecker
{
    ...
    /// 面含点检查
    /// </summary>
    public class GeoCheckAreaContainsPoint : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //面含点检查，参数为43
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 43, out eError);
                if (eError != null)
                {
                ...
                AreaTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTAreaContainPoint, out eError);
                if (eError != null)
                {
                    eError = new Exception("面含点检查失败！" + eError.Message);
                    return;
                }
        ...
        public void DataCheckRealize_DataErrTreat(object sender, DataErrTreatEvent e)
        {
            ...
            e.ErrInfo.FunctionName = "面含点检查";
            e.ErrInfo.ErrID = enumErrorType.面含点检查.GetHashCode();
