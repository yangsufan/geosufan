...
namespace GeoDataChecker
{
    ...
    /// 同层线相交检查
    /// </summary>
    public class GeoPolylineIntersectCheck : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //同层线相交检查
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 15, out eError);
                if (eError != null)
                {
                ...
                SpecialFeaClsTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTLineNoIntersection, out eError);
                if (eError != null)
                {
                    eError = new Exception( "同层线相交检查失败！" + eError.Message);
                    return;
                }
        ...
        public void DataCheckRealize_DataErrTreat(object sender, DataErrTreatEvent e)
        {
            ...
            e.ErrInfo.FunctionName = "同层线相交检查";
            e.ErrInfo.ErrID = enumErrorType.同层线相交检查.GetHashCode();
