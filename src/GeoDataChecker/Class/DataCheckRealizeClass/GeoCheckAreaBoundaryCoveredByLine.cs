...
namespace GeoDataChecker
{
    ...
    /// 面边界线重合检查
    /// </summary>
    public class GeoCheckAreaBoundaryCoveredByLine : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //面边界线重合检查，参数为44
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 44, out eError);
                if (eError != null)
                {
                ...
                AreaTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTAreaBoundaryCoveredByLine,out eError);
                if (eError != null)
                {
                    eError = new Exception("面边界线重合检查失败！" + eError.Message);
                    return;
                }
        ...
        public void DataCheckRealize_DataErrTreat(object sender, DataErrTreatEvent e)
        {
            ...
            e.ErrInfo.FunctionName = "面边界线重合检查";
            e.ErrInfo.ErrID = enumErrorType.面边界线重合检查.GetHashCode();
