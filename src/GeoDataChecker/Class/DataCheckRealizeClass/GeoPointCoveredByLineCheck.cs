...
namespace GeoDataChecker
{
    ...
    /// 点搭线检查
    /// </summary>
    public class GeoPointCoveredByLineCheck : IDataCheckRealize
    {
        ...
        public void OnDataCheck()
        {
            ...
            foreach (string name in featureDatasetNames)
            {
                ...
                //点搭线检查，参数ID为11
                DataTable mTable = TopologyCheckClass.GetParaValueTable(featureDataset, pSysTable, 11, out eError);
                if (eError != null)
                {
                ...
                AreaTopoCheck2(Hook, featureDataset, mTable, esriTopologyRuleType.esriTRTPointCoveredByLine, out eError);
                if (eError != null)
                {
                    eError=new Exception("点搭线检查失败！" + eError.Message);
                    return;
                }
        ...
        public void DataCheckRealize_DataErrTreat(object sender, DataErrTreatEvent e)
        {
            ...
            e.ErrInfo.FunctionName = "点搭线检查";
            e.ErrInfo.ErrID = enumErrorType.点搭线检查.GetHashCode();
