...
namespace GeoDBIntegration
{
    ...
    public static class ProjectXml
    {
        ...
        /// <summary>
        /// 根据数据库工程ID 获取数据库连接信息XmlElememt
        /// </summary>
        /// <param name="xmlDoc">对应的数据库的XMl文件</param>
        /// <param name="in_iProID">数据库的ID</param>
        /// <returns>XmlElement，找不到或内部异常返回NULL</returns>
        public static XmlElement GetProjectInfo(XmlDocument xmlDoc, long in_iProID)
        {
            XmlElement rootElem = xmlDoc.DocumentElement;
            string curProID = in_iProID.ToString();
            if (curProID != "")
            {
                //若存在工程，则遍历工程节点，获得属性信息并挂在树节点上
                foreach (XmlNode aXmlNode in xmlDoc.DocumentElement.ChildNodes)
                {
                    XmlElement aElement = aXmlNode as XmlElement;
                    //工程项目ID
                    string proID = aElement.GetAttribute("编号").Trim();
                    if (proID != curProID) continue;
                    else return aElement;                 
                 }                
            }
            return null;
        }
